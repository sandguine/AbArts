...
for i_file=1:n_files
    ...
    clearvars name_maintask_current sub_id_current trial_type_current trial_type_1or2_current  ...
        Response_current button_order_current response_meaning_current RT_current_current image_path_current;

    
    folder_name=['psiTurk-artTask-v' num2str(i_file, '%d')]
    
    
%% Initialize variables.
    if boolean(strfind(pwd, 'sandy'))
        filename=['/Users/sandy/Dropbox/Caltech/AbArts/ArtTask/', folder_name, '/trialdata.csv'];
    ...
    elseif boolean(strfind(pwd, 'miles'))
        ...
        filename=['/Users/miles/Dropbox/AbArts/ArtTask/', folder_name, '/trialdata.csv'];
    else
       %  filename = 'D:/Dropbox/AbArts/ArtTask/data/180424trialdata.csv';
         ...
         filename=['D:\Dropbox\AbArts\ArtTask\', folder_name, '\trialdata.csv'];
    end
   ...
   formatSpec = '%q%f%f%q%[^\n\r]';
    ...
    %% Clear temporary variables
   % clearvars filename delimiter formatSpec fileID dataArray ans ;
    %clearvars(name_maintask_current,sub_id_current,trial_type_current,trial_type_1or2_current, Response_current...
     %   ,button_order_current,response_meaning_current, RT_current_current,image_path_current);
    ...
    name_maintask_current=name_trial(nonsurvey_trial);  %% this contains main task
    ...
    name_maintask_current= cellstr(name_maintask_current);




    uniq_names=unique(name_maintask_current,'stable');
    l_matrix=length(infos_maintask);


    for s=1:l_matrix
        z=name_maintask_current{s,1};
        for j=1:length(uniq_names)
            if string(z) == string(uniq_names{j})
                sub_id_current(s,1)=j;
            end
        end
    end
    ...
    for i=1:l_matrix
        ...
        Response_current=[Response_current;current_response];

        if contains(char(x),"[I know., I don't know.]")
            button_order_current{i,1}='YN';
            trial_type_current{i,1}='knowledge';
            trial_type_1or2_current(i,1)=1;
        ...
        elseif contains(char(x),"[I don't know., I know.]")
            ...
            button_order_current{i,1}='NaN';
            trial_type_current{i,1}='rating';
            trial_type_1or2_current(i,1)=2;

            current_response_meaning = current_response;
        end

        response_meaning_current(i,1)=current_response_meaning;


        q=extractBetween(x,"images",",");

        image_path_current{i,1}=q;
    ...
    name_maintask=[name_maintask;name_maintask_current];
    ...
    sub_id=[sub_id;maxid+sub_id_current];
    trial_type=[trial_type;trial_type_current];
    trial_type_1or2=[trial_type_1or2;trial_type_1or2_current];
    Response=[Response;Response_current];
    button_order=[button_order;button_order_current];
    response_meaning=[response_meaning;response_meaning_current];
    RT=[RT;RT_current];
    image_path=[image_path;image_path_current];
    
    
end
    %% to do%% do the same for all elements and define []. 
    
maintask_table=table(name_maintask,sub_id,trial_type,trial_type_1or2, Response,button_order,response_meaning, RT,image_path); %% sub_id is wrong


uniq_names=unique(name_maintask,'stable');   %% not sure if the order matters

name_file_1='main_task_all_v1';
...
%     else
%             
%         n_trials_completed=length(find(sub_id_current==index_in_uniq_names));
