...
[x_exclude_1,y1]=find(x_sub==min(x_sub(:,1)));

x_sub(x_exclude_1,:)=[];


[x_exclude_2,y1]=find(x_sub==min(x_sub(:,7)));
...
n_regressor=length(index_to_analyze);
n=n_regressor;
Q=dec2bin(2^n-1:-1:0)-'0';

Q_to_test=Q(index_to_test, :);


load index_to_test_2.mat

Q_to_test_2=Q_to_test(index_to_test_2,:);
%n_model=2^n_regressor;

n_model=size(Q_to_test_2,1);

for i_sub=1:length(list_id)
%for    i_sub=1:200
     current_id=list_id(i_sub)     
     
     if length(find(sub_id_rating==current_id))>10  && i_sub ~=x_exclude_1 && i_sub ~=x_exclude_2
         current_image_index_list=rated_image_index(sub_id_rating==current_id);
         current_feature_list=image_features(current_image_index_list,:);

         current_feature_list_new=current_feature_list(:,index_to_analyze);

         current_response_list=response_r(sub_id_rating==current_id);

         med_response=median(current_response_list);

         res_normalized=sign(current_response_list-med_response);

          non_nan_index=~any(isnan(current_feature_list_new),2);
          non_nan_x=current_feature_list(non_nan_index,:);
          non_nan_x=double(non_nan_x);
          current_response_list=current_response_list(non_nan_index);

          clearvars  model_X

          for model_i =1:n_model

              current_model_input=logical(Q_to_test_2(model_i,:));



              model_X{model_i}=non_nan_x(:,current_model_input);
          end
          ...
          for model_i =1:n_model
             ...
             % size(X)
              ...
              l_res=length(current_response_list);
              y_to_test=NaN(1,l_res-1);
              y_predcted=NaN(1,l_res-1);

              for j=1:length(current_response_list)

                  Y_train=current_response_list;
                  X_train=X;

                  y_to_test(j)=current_response_list(j);
                  X_test=X_train(j,:);
                  ...
                  y_predcted(j) = predict(mdl,X_test);
              end

              R = corrcoef(y_to_test,y_predcted) ;
              Rs(model_i,i_sub)=R(1,2);
          end
...
save Rs_v4 Rs index_to_analyze Q_to_test_2
...
for j=1:n_model
    subplot(3,3,j)
    histogram(Rs(j,:),-0.7:0.02:0.7)
    
    [Ns,edges] = histcounts(Rs(j,:),-0.7:0.05:0.7);
    
    modes(j)=edges(Ns==max(Ns))
end


figure(11)
plot(sum(Rs==max(Rs),2))
