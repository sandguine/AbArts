...
load([savdir '/', 'main_task_long_v2', '.mat']);
...
if indexing
    ...
    name_file_2='image_index_long_task_v2';
...
load('Rs_v4.mat', 'index_to_analyze')
...
for i_sub=1:length(list_id)
     ...
     [bs2(i_sub,:), bs2_int{i_sub},~,~, stats{i_sub}] = regress(current_response_list(non_nan_index,1),X2)  ;
      ...
      X3=  [zscore(non_nan_x(:, index_to_analyze)),ones(size(non_nan_x,1),1)]; 
      
     
      
      
      [bs4(i_sub,:), bs4_int{i_sub},~,~, stats4{i_sub}] = regress(current_response_list(non_nan_index,1),X3)  ;
     ...
     bs3(i_sub,:)= regress(zscore(current_response_list(non_nan_index,1)),zscore(non_nan_x))'  ;
      ...
      response_to_analyze =current_response_list(non_nan_index,1); 
      response_to_analyze=zscore(response_to_analyze);
      
      
       X=X3;

      l_res=length( response_to_analyze);
      y_to_test=NaN(1,l_res-1);
      y_predcted=NaN(1,l_res-1);

      for j=1:length( response_to_analyze)

          Y_train= response_to_analyze;
          X_train=X;

          y_to_test(j)= response_to_analyze(j);
          X_test=X_train(j,:);
