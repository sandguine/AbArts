...
image_name_task_x = cellfun(@(x) extractAfter(extractAfter(x,'/'),'/'),image_x);
...
unique_images_task= unique(image_name_task_x , 'stable');
...
if indexing
    ...
    for i=1:length(image_name_task_x)

        current_image=image_name_task_x{i};
    ...
    name_file_2='image_index_familiarity_task';
...
for j=1:4
    r=response_x(image_categories(:,j)==1);
  ...
  %  if j==1
     ...
     %   ax.YTick = [0,1];    
       ...
       % ylim([0,1])
        ...
        ax.XTickLabel = {'Impressionism','AbstractArt','ColorFieldP','Cubism'};
