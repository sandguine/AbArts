...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 25 18:03:13 2018
...
def getInfo(dirName):
    ...
    for file in glob.glob("*.jpg"):
        ...
        im = Image.open(file)
        width, size = im.size
        infoList.append(im.info)
        sizeList.append(im.size)
    for file in glob.glob("*.bmp"):
        # print('{stimulus: \"img/' + dirName + '/' + file + '\"}, \n')
        im = Image.open(file)
        width, size = im.size
...
def getDPIs():
    ...
    allArts = ["abstract", "colorFields", "cubism", "impressionism", "leslie"]
    for artStyle in allArts:
        artList = getDPI(artStyle)
        #artChunk = chunk(artList, numChunks)
        allDPIs.append(artList)
    return allDPIs
...
def getInfos():
    ...
    allArts = ["abstract", "colorFields", "cubism", "impressionism", "leslie"]
    for artStyle in allArts:
        artList = getInfo(artStyle)
        #artChunk = chunk(artList, numChunks)
        allInfos.append(artList)
    return allInfos
...
def getSizes():
    ...
    allArts = ["abstract", "colorFields", "cubism", "impressionism", "leslie"]
    for artStyle in allArts:
        sizeList = getSize(artStyle)
        #artChunk = chunk(artList, numChunks)
        allSizes.append(sizeList)
    return allSizes
